...
oppia.factory('QuestionChangeListService', [
  '$rootScope', '$log', 'AlertsService', 'QuestionDataService',
  'AutosaveQuestionInfoModalsService',
  function(
      $rootScope, $log, AlertsService, QuestionDataService,
      AutosaveQuestionInfoModalsService) {
    ...
    // Temporary buffer for changes made to the question.
    var questionChangeList = [];
    ...
    var autosaveChangeListOnChange = function(questionChangeList) {
      ...
      QuestionDataService.autosaveChangeList(
        ...
        function(response) {
          if (!response.data.is_version_of_draft_valid) {
            if (!AutosaveQuestionInfoModalsService.isModalOpen()) {
              AutosaveQuestionInfoModalsService.showVersionMismatchModal(
                questionChangeList);
            }
          }
        },
        function() {
          ...
          $log.error(
            ...
            JSON.stringify(questionChangeList));
          if (!AutosaveQuestionInfoModalsService.isModalOpen()) {
            AutosaveQuestionInfoModalsService
    ...
    var addChange = function(changeDict) {
      ...
      questionChangeList.push(changeDict);
      undoneChangeStack = [];
      autosaveChangeListOnChange(questionChangeList);
    };
    ...
    return {
      ...
      discardAllChanges: function() {
        questionChangeList = [];
        undoneChangeStack = [];
        QuestionDataService.discardDraft();
      },
      /**
       * Saves a change dict that represents a change to an question
       ...
       */
      editQuestionProperty: function(backendName, newValue, oldValue) {
        if (!ALLOWED_EXPLORATION_BACKEND_NAMES.hasOwnProperty(backendName)) {
          AlertsService.addWarning(
            'Invalid question property: ' + backendName);
          return;
        }
        addChange({
      ...
      getChangeList: function() {
        return angular.copy(questionChangeList);
      },
      isQuestionLockedForEditing: function() {
        return questionChangeList.length > 0;
      },
      /**
      ...
      undoLastChange: function() {
        if (questionChangeList.length === 0) {
          AlertsService.addWarning('There are no changes to undo.');
          return;
        }
        var lastChange = questionChangeList.pop();
