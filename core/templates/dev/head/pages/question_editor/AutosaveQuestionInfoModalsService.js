...
oppia.factory('AutosaveQuestionInfoModalsService', [
  '$log', '$uibModal', '$timeout', '$window',
  'QuestionDataService', 'LocalQuestionStorageService',
  ...
  function(
    ...
    return {
        ...
        $uibModal.open({
          ...
          controller: ['$scope', function($scope) {
            ...
            $scope.discardChanges = function() {
              QuestionDataService.discardDraft(function() {
                _refreshPage(20);
              });
            };
            ...
            if ($scope.hasLostChanges) {
              // TODO(sll): This should also include changes to question
              // properties (such as the question title, category, etc.).
      ...
      showLostChangesModal: function(lostChanges, questionId) {
        $uibModal.open({
          templateUrl: UrlInterpolationService.getDirectiveTemplateUrl(
            '/pages/question_editor/lost_changes_modal_directive.html'),
          // Prevent modal from closing when the user clicks outside it.
          backdrop: 'static',
